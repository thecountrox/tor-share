...
class SignalingClient extends EventEmitter {
  constructor(serverUrl, options = {}) {
    ...
    console.log(`SignalingClient created with serverUrl: ${this.serverUrl}, bypassTor: ${this.options.bypassTor}`);
  }
  ...
  async connect(retries = 3, retryDelay = 2000) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        lastError = error;
        console.error(`Connection attempt ${attempt + 1}/${retries} failed:`, error);
        
        if (error.message && error.message.includes('ECONNREFUSED')) {
          console.error('Tor may not be running or accessible. Check Tor status before retrying.');
        }
        
        // If we've tried the max number of times, throw error
        if (attempt >= retries - 1) {
          throw new Error(`Failed to connect to signaling server after ${retries} attempts: ${lastError.message}`);
        }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.socket.on('connect', () => {
          console.log('[CLIENT] Connected to signaling server at', this.serverUrl);
          this.discover();
          resolve();
        });
        ...
        this.socket.on('clients', (clientList) => {
          console.log(`[CLIENT ${this.clientId}] Received client list from server with ${clientList.length} clients`);
          if (clientList.length > 0) {
            console.log(`[CLIENT ${this.clientId}] Available clients: ${clientList.join(', ')}`);
          } else {
        ...
        const timeoutId = setTimeout(() => {
          ...
          if (this.socket && !this.socket.connected) {
            ...
            const destroyedError = new Error('Socket was destroyed before connection was established');
  ...
  discover() {
    if (this.socket) {
      console.log(`[CLIENT ${this.clientId}] Requesting client list from signaling server`);
  ...
  startPeriodicDiscovery(interval = 5000) {
    ...
    console.log(`[CLIENT ${this.clientId || 'unknown'}] Starting periodic discovery every ${interval}ms`);
